...
```{r, message = FALSE}
hoteles_df <- read_csv("./datos/hoteles-entrena.csv", show_col_types = FALSE) |>
...
params <- parameters(penalty(range = c(-8,2), trans = log10_trans()),
                            mixture(range = c(0,1)))

grid <- grid_regular(params, levels = c(penalty = 75, mixture = 25))
...
```{r, message = FALSE}
hoteles_test <- read_csv("./datos/hoteles-prueba.csv", show_col_types = FALSE)
